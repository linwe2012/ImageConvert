/* camera class - deal with camera movement
* Oirginal Author: https://learnopengl.com/
* Modified by Leon Lin by adding budge camera function
*/
...
public:
	...
	void ProcessKeyboard(Camera_Movement direction, float deltaTime)
	{
		...
		if (MovementConstraint == CAMERA_3D) 
			Proceed = Front;
		else Proceed = Up;

		if (direction == FORWARD)
			Position += Proceed * velocity;
	...
	void adjustMovementStyle(int movement)
	{
	...
	void BudgeCamera(float deltaTime, float time, int function = GLGH_SMOOTH_COS, float toX=0.0f, float toY=0.0f, float toZ=1.0f, float toYaw=-90.0f, float toPitch=-0.0f, float toZoom=45.0f, bool constrainPitch=true) 
	{
		static float Ax = 0;
		static float Ay = 0;
		static float Az = 0;
